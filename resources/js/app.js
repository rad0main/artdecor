...
document.addEventListener('alpine:init', () => {
    ...
    Alpine.data('catalogGrid', () => ({
        ...
        toggleFavorite(imageId) {
            window.dispatchEvent(new CustomEvent('toggle-favorite', { detail: { imageId } }));
        },

        isFavorite(imageId) {
            // This will be connected to the favorites state
            return false;
        },
    }));
    ...
    Alpine.data('worksGrid', () => ({
        ...
        loadMore() {
            if (!this.loading && this.hasMore) this.loadWorks();
        },
    }));
    ...
    Alpine.data('primerka', () => ({
        ...
        async init() {
            ...
            if (this.colors.length > 0) {
                const defaultColor = this.colors.find(c => c.slug === 'beige') || this.colors[0];
                this.topColor = defaultColor.id;
                this.bottomColor = defaultColor.id;
                this.selectedTopColor = defaultColor.hex;
                this.selectedBottomColor = defaultColor.hex;
            }
        ...
        async loadImages() {
            ...
            try {
                const { data } = await axios.get('/api/catalog', {
                    params: { category: this.selectedCategory, limit: 50 }
                });
        ...
        openOrderForm() {
            ...
            window.dispatchEvent(new CustomEvent('open-modal', { detail: 'order-form' }));
        },
        ...
        async submitOrder() {
            try {
                ...
                const msg = e.response?.data?.message || 'Ошибка при отправке. Пожалуйста, попробуйте позже.';
                alert(msg);
            }
        },
    }));
